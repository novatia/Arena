namespace VUDK.Extensions.Finders
{
    ...
    public static class FinderExtension
    {
        ...
        /// <param name="closest">Closest gameobject.</param>
        /// <param name="excludeSelf">Indicates whether to exclude the self gameobject from the search.</param>
        /// <returns>True if the gameobject was found, False if not.</returns>
        public static bool TryGetClosestGameObjectWithTag(this GameObject self, string tag, out GameObject closest, bool excludeSelf = true)
        {
            ...
            if (gameObjects.Length > 0)
            {
                if (excludeSelf)
                {
                    List<GameObject> gameObjectList = new List<GameObject>(gameObjects);
        ...
        /// <summary>
        /// Try to get the closest gameobject in an array of gameobjects found with a specific component.
        /// </summary>
        ...
        /// <param name="excludeSelf">Indicates whether to exclude the self gameobject from the search.</param>
        /// <returns>True if the gameobject was found, False if not.</returns>
        public static bool TryGetClosestWithComponent<T>(this GameObject self, out GameObject closest, bool excludeSelf = true) where T : Component
        {
            ...
            if (components.Length > 0)
            {
                if (excludeSelf)
                {
                    components = components.Where(c => c.gameObject.GetInstanceID() != self.GetInstanceID()).ToArray();
                }

                if (components.Length > 0)
                {
                    closest = GetClosestGameObject(self, components.Select(c => c.gameObject).ToArray());
                    return true;
                }
            }
